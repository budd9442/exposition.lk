...
const PartnershipCard: React.FC<PartnershipCardProps & { onReadMore: () => void }> = ({ partnership, isLarge = false, onReadMore }) => (
  <motion.div
    layout
...
const PartnershipTree: React.FC = () => {
  ...
  const [modalPartnership, setModalPartnership] = useState<null | Partnership>(null);
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-center mb-8">
              {partnerships[activeTab].map((p) => (
                <PartnershipCard key={p.id} partnership={p} isLarge onReadMore={() => setModalPartnership(p)} />
              ))}
            </div>
          ...
          {modalPartnership && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
              <div className="bg-gray-900 rounded-2xl p-8 max-w-lg w-full relative shadow-2xl border-2 border-[#e3c767]">
                <button
                  onClick={() => setModalPartnership(null)}
                  className="absolute top-4 right-4 p-2 rounded-full bg-black/40 hover:bg-black/70 text-white"
                  aria-label="Close"
                >
                  <X className="w-6 h-6" />
                </button>
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-gray-300 mb-4">{modalPartnership.description}</p>
